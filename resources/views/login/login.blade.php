...
<body class="min-h-screen flex items-center justify-center bg-white-500">
    <div class="relative py-3 sm:max-w-xl sm:mx-auto">
        <div class="absolute inset-0 bg-gradient-to-r from-yellow-300 to-yellow-600 shadow-lg transform -skew-y-6 sm:skew-y-0 sm:-rotate-6 sm:rounded-3xl">
        </div>
        <div class="relative px-4 py-10 bg-white shadow-2xl sm:rounded-3xl sm:p-20 w-full max-w-md text-center">
            <!-- Logo -->
            <a href="{{ route('home') }}">
                <img src="{{ asset('img/logo.png') }}" alt="K0munitat Logo" class="mx-auto mb-10 hover:scale-110" style="max-width: 250px;">
            </a>
            <form method="POST" action="{{ route('inicia-sesion') }}">
                ...
                <div class="mb-4 relative">
                    <input autocomplete="off" id="email" name="email" type="text" class="peer placeholder-transparent h-10 w-full border-b-2 border-gray-300 text-gray-900 focus:outline-none focus:border-yellow-600" placeholder="Email address" />
                ...
                <div class="mb-4 relative">
                    ...
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 eye-icon" id="togglePassword">
                        <i class="far fa-eye-slash text-gray-600"></i>
                    </div>
                </div>
                <div class="mb-4">
                    <a href="{{ route('auth.redirect1') }}" class="w-full bg-red-600 text-white rounded-md px-2 py-1 block">
                        <i class="fab fa-google mr-2"></i> Log in with Google
                    </a>
                </div>
                <!-- Botón de iniciar sesión con Facebook -->
                <div class="mb-4">
                    <a href="{{ route('auth.redirect') }}" class="w-full bg-blue-600 text-white rounded-md px-2 py-1 block">
