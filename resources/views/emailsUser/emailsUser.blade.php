...
@section('content')
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div id="activeMessagesButton">
            <button onclick="showDeletedMessages()" style="background-color: #808080"
                class="text-white px-4 py-2 rounded-md mb-4">Deleted messages</button>
        </div>
                        ...
                        @foreach ($activeEmails as $email)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button
                                        onclick="showMore(event, '{{ $email->sender_name }}', '{{ $email->sender_email }}', '{{ $email->message }}', '{{ $email->id }}')"
                                        style="background-color: #808080"
                                        class="text-white px-4 py-2 rounded-md w-32 h-full">Show More</button>
                                    <button onclick="confirmDelete(event, {{ $email->id }})"
                                        style="background-color: #E51C1C"
                                        class="text-white px-4 py-2 rounded-md border mr-2 w-32 h-full">Delete</button>
                                </td>
                            </tr>
                        @endforeach
                    @endif
        ...
        <div id="deletedMessages" class="hidden">
            <button onclick="showActiveMessages()" style="background-color: #808080"
                class="text-white px-4 py-2 rounded-md mb-4">Active messages</button>
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                ...
                <tbody>
                    @if ($deletedEmails->isEmpty())
                        <tr>
                            <td colspan="3" class="px-6 py-4 text-center">There aren't deleted messages.</td>
                        </tr>
                    @else
    ...
    @foreach ($activeEmails as $email)
        ...
        <div id="showMoreModal-{{ $email->id }}"
            class="mt-10 hidden fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
            <div class="bg-white p-8 rounded shadow-md max-w-2xl overflow-y-auto" style="max-height: 80vh;">
                <div class="flex items-center mb-4">
                    @if (isset($email->profile_image))
                        <img src="{{ $email->profile_image }}" alt="User Image" class="w-10 h-10 rounded-full">
                    @else
                        <!-- Si no hay $email definido, muestra una imagen genérica -->
                        <img src="/placeholder-image.jpg" alt="User Image" class="w-10 h-10 rounded-full">
                    @endif
                    <div class="ml-4">
                        <h2 class="text-lg font-semibold">Sender: <span id="senderName-{{ $email->id }}">{{ $email->sender_name }}</span>
                        </h2>
                        <p class="text-gray-600" id="senderEmail-{{ $email->id }}">{{ $email->sender_email }}</p>
                    </div>
                </div>

                <div class="mb-4">
                    <p class="font-semibold">Message:</p>
                    <p id="message-{{ $email->id }}">{{ $email->message }}</p>
                </div>
                <!-- Formulario de respuesta -->
                <form id="replyForm" action="{{ route('reply.message-2') }}" method="POST">
                    @csrf
                    ...
                    <div class="mt-4 flex justify-end">
                        <button type="submit" style="background-color: #64a858"
                            class="text-white px-4 py-2 rounded-md mr-2">Send</button>
                        <button type="button" onclick="closeShowMoreModal({{ $email->id }})" style="background-color: #3d3c3b"
                            class="text-white px-4 py-2 rounded-md">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function showMore(event, senderName, senderEmail, message, id) {
            event.stopPropagation();
        ...
        function closeShowMoreModal(id) {
            document.getElementById('showMoreModal-' + id).classList.add('hidden');
        }
    </script>
