...
@section('content')
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($activeMessages as $message)
                        <tr onclick="openMessageModal('{{ $message->message }}')">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <button onclick="replyToMessage(event, '{{ $message->sender_name }}', {{ $message->id }})" class="text-blue-600">
                                    Reply
                                </button>
                            </td>
                        </tr>
                    ...
                    @foreach ($deletedMessages as $message)
                        <tr onclick="openMessageModal('{{ $message->message }}')">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <form id="restoreForm{{ $message->id }}" method="POST" action="{{ route('restoreAdmin.message', $message->id) }}">
                                    @csrf
    ...
    <div id="replyModal" class="hidden fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
        <div class="bg-white p-8 rounded shadow-md">
            <h2 class="text-lg font-semibold mb-4">Reply to: <span id="replyTo"></span></h2>
            <form id="replyForm" action="{{ route('reply.message') }}" method="POST">
                ...
                <div class="mt-4 flex justify-end">
                    ...
                    <button type="button" onclick="closeReplyModal()" class="bg-gray-500 text-white px-4 py-2 rounded-md">Cancel</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function replyToMessage(event, senderName, messageId) {
            event.stopPropagation();
