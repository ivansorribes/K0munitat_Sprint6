...
const EventCalendar = () => {
  ...
  const handleSelect = ({ start }) => {
    // Aquí puedes verificar si el usuario tiene el rol de superAdmin

    // Si el usuario es superAdmin, abres la modal
    if (user.role === "superAdmin") {
      ...
      // Si el usuario no es superAdmin, puedes mostrar un mensaje o realizar otra acción
      console.log("No tienes permiso para crear eventos.");
    }
  };
  ...
  const handleFormSubmit = (values, { resetForm } = {}) => {
    ...
    axios
      .post("/eventsList", sanitizedValues, {
        headers: {
  ...
  const validateForm = (values) => {
    ...
    if (
      values.start &&
      values.end &&
      moment(values.start).isAfter(values.end)
    ) {
