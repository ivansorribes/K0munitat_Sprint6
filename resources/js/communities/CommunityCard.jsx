...
const CommunityCard = ({ community, option, user }) => {
  ...
  if (option === "enter") {
    buttonComponent = (
      <ButtonSave label="Enter community" onClick={handleButtonClick} />
    );
  } else if (option === "send") {
    buttonComponent = (
      <ButtonEdit label="Send request" onClick={handleButtonClick} />
    );
  } else {
  ...
  return (
    <div className="bg-white p-6 rounded-md shadow-md flex flex-col justify-center items-center">
      ...
      >
        <h2
          className="text-2xl font-bold mb-1 text-center overflow-hidden"
      ...
      <a href={communityUrl} target="_blank" rel="noopener noreferrer">
        <div className="flex justify-center mb-1">{buttonComponent}</div>
      </a>
              ...
              <div className="modal-content py-4 text-left px-6">
                <div className="pb-3">
                  <p className="text-2xl font-bold text-center">
                    Send Request to Join {community.name}
                  </p>
                </div>
                <div className="my-5">
                  {/* Contenido del formulario o mensaje aquí */}
                </div>
